...
## Remote
From a remote RPi to a local Linux machine (e.g. a second RPi with adequate disk capacity) or to a Windows machine with a Windows Subsystem for Linux (WSL) setup.
...
```
    ...
    NOTE: WSL may require additional setup including adding the following to you c:\%USER_PROFILE%\.wslconfig file
...
```
    ...
    Note: For WSL you may need to reference a local network name versus the IP address.
